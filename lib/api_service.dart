...
class ApiService {
  ...
  Future<String?> submitReview({
    ...
    final token = await storage.read(key: 'token'); // Retrieve token for authentication
    ...
    if (response.statusCode == 201) {
      return null; // Review submitted successfully
    } else {
  ...
  Future<Map<String, dynamic>> fetchSupermarketBySlug(String slug) async {
    final response = await http.get(Uri.parse('$baseUrl/supermarkets/slug/$slug'));
